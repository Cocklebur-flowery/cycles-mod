...
- Cycles 5.2 已通过 C ABI v6 接入 Java Foreign Function & Memory API；v6 在 v5 Section 协议之上增加设置、能力和诊断结构，不改变既有场景结构的布局。
...
`runNativeSmoke` 会构造一个带 UV、彩色纹理与 Alpha Clip 的小型网格场景，验证 ABI v6 能力/诊断、全部 7 个 Pass、Combined 恢复、检测到的 OptiX 降噪，以及 Section 创建、修改和删除，然后输出实际后端、设备、分辨率和帧校验和。
...
Minecraft SectionCompiler（16³ Section、流体、NeoForge 追加几何）
  ...
  -> NativeBridge（ABI v6：共享资源 + Section 流送 + 设置/能力/诊断）
  -> CyclesEngine 后台场景请求
...
ABI v6 保留旧的整场景入口和 v5 的 `CyclesBridgeSceneResources`、`CyclesBridgeSection`、`CyclesBridgeFrame` 布局，并新增固定 208 字节的 `CyclesBridgeRenderSettings`、64 字节的 `CyclesBridgeCapabilities` 和 96 字节的 `CyclesBridgeDiagnostics`。共享材质/图集由 `reset_scene` 设置；区块通过 `upsert_section`、`remove_section` 进入暂存区，再由 `commit_scene` 原子发布。`render_frame` 只在帧 generation 改变时复制像素。Java 与 DLL 的 ABI 版本不一致时会在启用前拒绝运行。
