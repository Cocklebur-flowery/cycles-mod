...
- Cycles 5.2 已通过 C ABI v10 接入 Java Foreign Function & Memory API；v10 将轻量相机更新与成品帧拉取解耦，不改变既有场景、Section、设置和诊断结构体布局。
...
`runNativeSmoke` 会构造一个带 UV、彩色纹理与 Alpha Clip 的小型网格场景，验证 ABI v10 的独立相机更新、能力/诊断、实际/目标 sample、帧与场景更新计数、全部 7 个 Pass、Combined 恢复、检测到的 OptiX 降噪，以及 Section 创建、修改和删除，然后输出实际后端、设备、分辨率和帧校验和。
...
Minecraft SectionCompiler（16³ Section、流体、NeoForge 追加几何）
  ...
  -> NativeBridge（ABI v10：共享资源 + Section 流送 + 独立相机更新 + 设置/能力/采样、帧与场景遥测）
  -> CyclesEngine 后台场景请求
...
ABI v10 保留旧的整场景入口、v5 Section 布局和 v6 设置/能力字段；`CyclesBridgeRenderSettings` 固定 208 字节、`CyclesBridgeCapabilities` 固定 64 字节，`CyclesBridgeDiagnostics` 固定 240 字节。v7 字段报告实际/目标 sample、采样状态和 sample/s；v8 追加 Native 转换/复制与帧统计；v9 追加整 Section 映射提交、Cycles scene delta 和 render start 的计数及 last/EMA/max；v10 新增 `update_camera`，允许 Minecraft 每帧投递相机而不复制成品像素。共享材质/图集由 `reset_scene` 设置；区块通过 `upsert_section`、`remove_section` 进入暂存区，再由 `commit_scene` 原子发布。`render_frame` 仍兼容旧行为，只在帧 generation 改变时复制像素。Java 与 DLL 的 ABI 版本不一致时会在启用前拒绝运行。
