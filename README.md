...
- Cycles 5.2 已通过 C ABI v7 接入 Java Foreign Function & Memory API；v7 在 v6 设置/能力/诊断协议上追加实际采样遥测，不改变既有场景和 Section 结构的布局。
...
`runNativeSmoke` 会构造一个带 UV、彩色纹理与 Alpha Clip 的小型网格场景，验证 ABI v7 能力/诊断、实际/目标 sample、全部 7 个 Pass、Combined 恢复、检测到的 OptiX 降噪，以及 Section 创建、修改和删除，然后输出实际后端、设备、分辨率和帧校验和。
...
Minecraft SectionCompiler（16³ Section、流体、NeoForge 追加几何）
  ...
  -> NativeBridge（ABI v7：共享资源 + Section 流送 + 设置/能力/诊断/采样遥测）
  -> CyclesEngine 后台场景请求
...
ABI v7 保留旧的整场景入口、v5 Section 布局和 v6 设置/能力字段；`CyclesBridgeRenderSettings` 固定 208 字节、`CyclesBridgeCapabilities` 固定 64 字节，`CyclesBridgeDiagnostics` 扩展为 112 字节并追加实际/目标 sample、采样状态和 sample/s。共享材质/图集由 `reset_scene` 设置；区块通过 `upsert_section`、`remove_section` 进入暂存区，再由 `commit_scene` 原子发布。`render_frame` 只在帧 generation 改变时复制像素。Java 与 DLL 的 ABI 版本不一致时会在启用前拒绝运行。
