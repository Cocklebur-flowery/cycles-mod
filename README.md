...
- Cycles 5.2 已通过 C ABI v8 接入 Java Foreign Function & Memory API；v8 在 v7 实际采样诊断上追加帧转换/复制遥测，不改变既有场景和 Section 结构的布局。
...
`runNativeSmoke` 会构造一个带 UV、彩色纹理与 Alpha Clip 的小型网格场景，验证 ABI v8 能力/诊断、实际/目标 sample、帧生产/复制计数、全部 7 个 Pass、Combined 恢复、检测到的 OptiX 降噪，以及 Section 创建、修改和删除，然后输出实际后端、设备、分辨率和帧校验和。
...
Minecraft SectionCompiler（16³ Section、流体、NeoForge 追加几何）
  ...
  -> NativeBridge（ABI v8：共享资源 + Section 流送 + 设置/能力/采样与帧管线遥测）
  -> CyclesEngine 后台场景请求
...
ABI v8 保留旧的整场景入口、v5 Section 布局和 v6 设置/能力字段；`CyclesBridgeRenderSettings` 固定 208 字节、`CyclesBridgeCapabilities` 固定 64 字节，`CyclesBridgeDiagnostics` 扩展为 176 字节。v7 字段报告实际/目标 sample、采样状态和 sample/s；v8 追加 Native 转换/复制 last/EMA/max、生产/复制/未更新轮询计数、复制字节与帧龄。共享材质/图集由 `reset_scene` 设置；区块通过 `upsert_section`、`remove_section` 进入暂存区，再由 `commit_scene` 原子发布。`render_frame` 只在帧 generation 改变时复制像素。Java 与 DLL 的 ABI 版本不一致时会在启用前拒绝运行。
